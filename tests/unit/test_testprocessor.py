import pytest
from dqlite.test_run import DQTestProcessor
...
def test_process_successful(mocker, mock_config, sample_dqtest):
    ...
    mocker.patch.object(dqlite.test_run, "get_connection", return_value=mock_conn)

    mock_validator = mocker.MagicMock()
    mock_validator.validate_metric.return_value = "GREEN"
    mocker.patch.object(dqlite.test_run, "get_validator", return_value=mock_validator)
...
def test_process_exception(mocker, mock_config, sample_dqtest):
    ...
    mocker.patch.object(dqlite.test_run, "get_connection", side_effect=Exception("DB Connection Failed"))
